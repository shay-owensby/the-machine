...
python3 $D/render_report.py \\
  --report analytics-insights/google-analytics/YYYY-MM-DD-google-analytics.md \\
  --analysis <..._analysis.json> \\
  --source google-analytics --project-root .
```
...
listed in `HEADLINE_KPIS` in `render_report.py` under the `google-analytics` key — add or
...
A Google Analytics 4 report carries the client's accent colour in its masthead rule, links
...
an unconfirmed accent is how a GA4 property report goes out in another company's colour.
